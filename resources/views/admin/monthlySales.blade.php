...
<body>
    <div class="rounded-lg shadow bg-base-200 drawer drawer-mobile">
        ...
        <h1 class="font-medium text-xl banner-text">&nbsp;This Month's Sales</h1>
        <div class="gen-button relative">
            <h1 class="font-medium text-xl banner-text">Total: RM {{$total}}</h1>
            <a href="{{ route('admin.exportMonth')}}" class="absolute top-0 right-0 rounded-l-none btn btn-primary" type="submit">Generate Report</a>
        </div>
        @if(isset($message))
            <span>{{$message}}</span>
        @endif
