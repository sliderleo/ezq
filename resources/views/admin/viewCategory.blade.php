...
<body>
    <div class="rounded-lg shadow bg-base-200 drawer drawer-mobile">
        ...
        <h1 class="font-medium text-xl banner-text">&nbsp;All Category</h1>    
        @if(isset($category))
        <table class="table">
            <thead>
                <tr>
                ...
                @foreach ($category as $item)
                    <tr>
                        ...
                        @else
                            ...
                            <td><input type="checkbox" data-id="{{$item->id}}" class="toggle toggle-primary" {{ $item->status ? 'checked' : '' }}></td>
                        @endif
                        <td><a href="{{url('admin/category-edit/'.$item->id)}}" class="btn btn-info">Edit</a></td>
                        <td><a href="{{url('admin/category-delete/'.$item->id)}}" class="btn btn-error">Delete</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
