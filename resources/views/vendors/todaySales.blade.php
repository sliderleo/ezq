...
<body>
    <div class="rounded-lg shadow bg-base-200 drawer drawer-mobile">
        ...
        <h1 class="font-medium text-xl banner-text">&nbsp;Today's Sales</h1>
        
        @if(count($receipt)>0) 
        <div class="gen-button relative">
          <h1 class="font-medium text-xl banner-text">Total: RM {{$total}}</h1>
          <a href="{{ route('vendor.getDaily')}}" class="btn btn-primary" type="submit">Generate Report</a>
        </div>
      ...
      @else
        <div class="container">
            <h1 class="font-medium text-xl banner-text">You don't have any sales for today yet!</h1>
        </div>
      @endif
    </div>
